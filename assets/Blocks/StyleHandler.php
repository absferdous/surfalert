...
final class StyleHandler {
    ...
    public function __construct() {
        add_action( 'admin_enqueue_scripts', [ $this, 'surfalert_pro_blocks_edit_post' ] );
        add_action( 'wp_ajax_surfalert_pro_write_block_css', [ $this, 'write_block_css' ] );
     ...
     */
    public function surfalert_pro_blocks_edit_post( $hook ) {
        $current_screen = get_current_screen();
        $post_type      = !empty( $current_screen->post_type ) ?  $current_screen->post_type : '';
        $dir = dirname( __FILE__ );
        if ( ($hook == 'post-new.php' || $hook == 'post.php') && $post_type == 'sa_bar_eb'   ) {
            wp_enqueue_script(
                'surfalert-pro-blocks-edit-post',
                SURFALERT_URL . 'blocks/style-handler/style-handler.js',
                array(
                    'lodash',
                    'wp-i18n',
                    'wp-element',
                ...
                filemtime( SURFALERT_PATH . 'blocks/style-handler/style-handler.js' ),
                true
            );
            wp_localize_script('surfalert-pro-blocks-edit-post', 'sa_style_handler', [
                'sth_nonce'   => wp_create_nonce( 'sa_style_handler_nonce' ),
    ...
    public function write_block_css() {
        if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'sa_style_handler_nonce' ) || ! current_user_can( 'manage_options' ) ) {
        ...
        if ( isset( $_POST['editorType'] ) && $_POST['editorType'] === 'edit-site' ) {
            $upload_dir      = wp_upload_dir()['basedir'] . '/sa-style/';
            $editSiteCssPath = $upload_dir . 'sa-style-' . $_POST['editorType'] . '.min.css';
            ...
            if ( ! empty( $css = $this->build_css( $block_styles ) ) ) {
                ...
                file_put_contents( $upload_dir . 'sa-style-' . abs( $_POST['id'] ) . '.min.css', $css );
            }
        }
    ...
    public function enqueue_frontend_css() {
        ...
        if ( ! empty( $post ) && ! empty( $post->ID ) ) {
            ...
            if ( file_exists( $upload_dir['basedir'] . '/sa-style/sa-style-' . $post->ID . '.min.css' ) ) {
                wp_enqueue_style( 'sa-block-style-' . $post->ID, $upload_dir['baseurl'] . '/sa-style/sa-style-' . $post->ID . '.min.css', [], substr( md5( microtime( true ) ), 0, 10 ) );
            } elseif ( function_exists( 'icl_object_id' ) ) {
                $default_language = apply_filters( 'wpml_default_language', null );
                $english_version  = icl_object_id( $post->ID, 'post', false, $default_language );
                if ( file_exists( $upload_dir['basedir'] . '/sa-style/sa-style-' . $english_version . '.min.css' ) ) {
                    wp_enqueue_style( 'sa-block-style-' . $english_version, $upload_dir['baseurl'] . '/sa-style/sa-style-' . $english_version . '.min.css', [], substr( md5( microtime( true ) ), 0, 10 ) );
                }
            }
            if ( function_exists( 'wp_is_block_theme' ) && wp_is_block_theme() && file_exists( $upload_dir['basedir'] . '/sa-style/sa-style-edit-site.min.css' ) ) {
                wp_enqueue_style( 'sa-fullsite-style', $upload_dir['baseurl'] . '/sa-style/sa-style-edit-site.min.css', [], substr( md5( microtime( true ) ), 0, 10 ) );
            }
        }
    }
