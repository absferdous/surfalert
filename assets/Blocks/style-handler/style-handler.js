...
(() => {
  ...
  const { editor_type } = sa_style_handler;
  ...
  const regexForBlockNameTest = /^surfalert\-pro\//g;
  ...
  const generateAndSendStyleToBackend = () => {
    ...
    // Only proceed if post type is 'sa_bar_eb'
    if (currentPostType !== 'sa_bar_eb') {
      return;
    }
        ...
        if (isFunction(isReusableBlock) && isReusableBlock(item)) {
          ...
          if (parsedBlocks) {
            for (const blockItem of parsedBlocks) {
              ...
              if (blockMeta && blockRoot === "surfalert_pro") {
          ...
          cssObjectMaker(innerBlocks);
        } else if (blockMeta && blockRoot === "surfalert_pro") {
          styles[blockId] = blockMeta;
        }
        if (innerBlocks.length > 0) {
          cssObjectMaker(innerBlocks);
    ...
    jQuery.ajax({
      ...
      data: {
        ...
        action: "surfalert_pro_write_block_css",
        nonce: sa_style_handler.sth_nonce,
      },
