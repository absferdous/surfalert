...
export default function Edit(props) {
  ...
  useEffect(() => {
    const BLOCK_PREFIX = "surfalert-inline";
    duplicateBlockIdFix({
      BLOCK_PREFIX,
      blockId,
      setAttributes,
  ...
  // style for Desktop
  const desktopStyles = `
    .${blockId}.surfalert-block-wrapper {
      display: flex;
      align-items: center;
      justify-content: ${nxWrapperAlign};
    }
    .${blockId}.surfalert-block-wrapper .sa-shortcode-notice {
    ...
    .${blockId}.surfalert-block-wrapper .sa-shortcode-notice a {
      ${titleTypographyDesktop}
      color: ${nxLinkColor};
    }
  `;
    ...
    <>
      ...
      <div {...useBlockProps()}>
        ...
        <ServerSideRender
          ...
          attributes={{ sa_id: attributes.sa_id, product_id: attributes.product_id, post_type:attributes.post_type }}
          className={`surfalert-block-wrapper ${blockId}`}
        />
      </div>
    </>
  );
