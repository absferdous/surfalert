...
class PostType
{
    ...
    public function admin_enqueue_scripts($hook)
    {
        ...
        if (strpos($hook, 'surfalert_page_') === false && strpos($hook, 'toplevel_page_sa-admin') === false) {
            return;
        }

        // This filter tells WordPress to treat our script as a module, which is required by Vite.
        add_filter('script_loader_tag', function ($tag, $handle, $src) {
            if ('surfalert-admin-app' === $handle) {
                $tag = '<script type="module" src="' . esc_url($src) . '"></script>';
            }
            return $tag;
        }, 10, 3);

        // Enqueue the new script from our 'build' directory.
        wp_enqueue_script(
            'surfalert-admin-app',
            SURFALERT_URL . 'build/admin.js',
            [], // Dependencies are handled by Vite and modern imports.
            SURFALERT_VERSION,
