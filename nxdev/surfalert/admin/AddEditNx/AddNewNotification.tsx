...
const AddNewNotification = (props) => {
        ...
        if (props?.location?.state?.duplicate) {
            ...
            nxHelper.get(`nx/${ID}`).then((res: any) => {
                if (res) {
                    ...
                    delete res.nx_id;
                    res.nx_id = null;
                    res.enabled = notificationxContext?.is_pro_active
                        ? true
                        : false;
                    ...
                    setTitle(res?.title + __(" - Copy", "surfalert"));
                    setIsLoading(false);
                }
            });
        }
    ...
    useEffect(() => {
        if (notificationxContext?.state?.redirect?.state?.type) {
            builder.setValues({
                type: notificationxContext?.state?.redirect?.state?.type,
                source: notificationxContext?.state?.redirect?.state?.source,
            });
        }
    }, []);
