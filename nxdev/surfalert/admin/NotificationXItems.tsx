...
export const NotificationXItems = (props) => {
    ...
    const [surfalert, setNotificationx] = useState([loading]);
    ...
    useEffect(() => {
        ...
        nxHelper
            .get(
                `nx?status=${status}&page=${currentPage}&per_page=${perPage}&s=${searchKey}`,
            ...
            .then((res: any) => {
                ...
                if (isArray(res?.posts) && isMounted.current) {
                    ...
                    setFilteredNotice(nxHelper.filtered(res?.posts, status));
                }
    ...
    React.useEffect(() => {
        setFilteredNotice(nxHelper.filtered(surfalert, status));
    }, [surfalert]);
                        ...
                        <div className="sa-no-items">
                            ...
                            {status == "all" ? (
                                <>
                                    ...
                                    <p>
                                        {__(
                                            `Seems like you haven’t created any notification alerts.`,
                                            "surfalert"
                                        )}
                                        <br />
                                        {parse(
                                            sprintf(
                                                __(
                                                    `Hit on %1$s"Add New"%2$s button to get started`,
                                                    "surfalert"
                                                ),
                                                "<b>",
                                                "</b>"
                                ...
                                <>
                                    ...
                                    <p>
                                        {__(
                                            `There’s no enabled Notification Alerts.`,
                                            "surfalert"
                                        )}
                                        <br />
                                        {parse(
                                            sprintf(
                                                __(
                                                    `Simply use the toggle switch to turn your notifications from %1$s"All SurfAlert"%2$s page.`,
                                                    "surfalert"
                                                ),
                                                "<b>",
                                                "</b>"
                                ...
                                <>
                                    ...
                                    <p>
                                        {__(
                                            "There’s no disabled Notification Alerts.",
                                            "surfalert"
                                        )}
                                    </p>
                                </>
