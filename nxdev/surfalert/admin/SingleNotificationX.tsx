...
const SingleNotificationX = ({
    ...
    return (
        ...
        >
            <td>
                <div className="sa-item-selector">
                    <input
                        type="checkbox"
                        name={`check-${id}`}
                        id={`check-${id}`}
            ...
            <td>
                <div className="sa-admin-type" title={item?.source_label}>
            ...
            <td>
                <div className="sa-admin-stats">
                    <NavLink
                        title={sprintf(
                            __("%s clicks", "surfalert"),
                            item?.clicks || 0
                        )}
                        to={{
                            pathname: "/admin.php",
                            search:
                                "?page=sa-analytics&comparison=views&nx=" + id,
                        }}
                    >
                        {/* translators: %d: Number of views for a Notification Alert. */}
                        {sprintf(__("%s views", "surfalert"), item?.views || 0)}
                    </NavLink>
                </div>
            </td>
            <td>
                <div className="sa-admin-date">
                    {isInTheFuture(updated_at.format())
                        ? __("Scheduled For", "surfalert")
                        : __("Published", "surfalert")}
                    <br />
                    <span className="sa-admin-publish-date">
                        {date(
