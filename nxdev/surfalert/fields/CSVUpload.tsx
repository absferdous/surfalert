...
const Media = (props) => {
    ...
    const handleMediaSelection = async (media) => {
        if (media.mime !== "text/csv") {
            nxHelper.swal({
                title: __("Invalid File Type!", "surfalert"),
                text: __(
                    "Please upload a CSV file to import custom notification data.",
                    "surfalert"
                ),
                iconHtml: `<img alt="SurfAlert" src="${builderContext.assets.admin}images/file-type.svg" />`,
                confirmButtonText: __("Close", "surfalert"),
                customClass: {
                    container: "sa-csv-modal-ift-container",
                    popup: "sa-csv-modal-ift-popup",
                    actions: "sa-delete-actions sa-csv-invalid-file-type",
                },
                confirmedCallback: () => {},
        ...
        try {
            ...
            if (itemCount > parseInt(nxContext?.cus_imp_limit)) {
                Swal.fire({
                    title: __("Import Limit Exceeded.", "surfalert"),
                    html: __(
                        `Your file contains more than ${parseInt(
                            nxContext?.cus_imp_limit
                        )} rows. Only the first ${parseInt(
                            nxContext?.cus_imp_limit
                        )} rows will be imported. Click <strong>"Continue"</strong> to proceed or <strong>"Cancel"</strong> to abort.`,
                        "surfalert"
                    ),
                    iconHtml: `<img alt="SurfAlert" src="${builderContext.assets.admin}images/file-type.svg" style="height: 85px; width:85px" />`,
                    showDenyButton: true,
                    iconColor: "transparent",
                    confirmButtonText: __("Continue", "surfalert"),
                    denyButtonText: __("Cancel", "surfalert"),
                    reverseButtons: true,
                    customClass: {
                        container: "sa-csv-modal-import-limit-container",
                        popup: "sa-csv-modal-import-limit-popup",
                        actions: "sa-delete-actions sa-csv-import-limit",
                    },
            ...
            nxToast.error(__("Error processing the CSV file", "surfalert"));
        }
    };
        ...
        for (let i = 0; i < totalChunks; i++) {
            ...
            if (progressBar) {
                ...
                progressBar.querySelector(
                    "span"
                ).innerText = `${progressValue}%`;
            }
        }

        setComplete(true);
        Swal.close();
        nxToast.info(__("CSV data imported successfully!", "surfalert"));
    };
        ...
        <div className="wprf-control wprf-media wprf-csv-upload">
                        ...
                        return (
                            <>
                                ...
                                <a
                                    ...
                                    download
                                >
                                    <FileIcon /> {__("Sample CSV", "surfalert")}
                                </a>
                            </>
                        );
                    }}
            ...
            {(progress > 0 || csv_upload_loader) && (
                <div className="progress-container">
                    <div id="sa-progress-bar" className="sa-progress-bar">
                        <span>0%</span>
                    </div>
                </div>
            )}
