...
const BetterRepeaterValueShow = (props) => {
    ...
    // @ts-ignore
    const fieldsArray = Object.values(fields).filter((field) =>
        visible_fields.includes(field?.name)
    );
    ...
    const onDelete = (event: Event) => {
        ...
        nxHelper.swal({
            html: `<div class="sa-gdpr-cookies-delete-modal">
                    ${binIcon} 
                    <h2>${__(
                    ...
                    <p>${sprintf(
                        __(
                            "The cookie <strong>%s</strong> will be permanently deleted. This cookie will no longer be displayed on your cookie list nor be blocked prior to receiving user consent.",
                            "surfalert"
                        ),
                        builderContext.values?.[parent]?.[index]?.cookies_id
                    )}</p>
                </div>`,
            showCancelButton: true,
            confirmButtonText: __("Delete", "surfalert"),
    ...
    return (
        <div
            className={`wprf-repeater-field ${discovered ? "discovered" : ""}`}
        >
            <div className="wprf-repeater-inner-field">
                {fieldsArray.map((field, fieldIndex) => {
                    // @ts-ignore
                    if (
                        !builderContext.values?.[parent]?.[index]?.[field?.name]
                    ) {
                        return;
                    }
                    return (
                        <div
                            className="wprf-repeater-inner-field-item"
                            key={"wprf-repeater-inner-field" + fieldIndex}
                        >
                            {/* @ts-ignore  */}
                            <span>{field?.label}</span>
                            {/* @ts-ignore  */}
                            {discovered && field?.name == "cookies_id" && (
                                <span>{__("Discovered", "surfalert")}</span>
                            )}
                            {/* @ts-ignore  */}
                            <p>
                                {
                                    builderContext.values?.[parent]?.[index]?.[
                                        field?.name
                                    ]
                                }
                            </p>
                ...
                {/* {!isDefault && ( */}
                <div className="sa-action-toggle-wrapper" ref={buttonRef}>
                    <a
                    ...
                    {action && (
                        <div className="sa-cookies-list-action">
                            {/*  || item?.elementor_id */}
                            <ul id="sa-admin-actions-ul">
                                ...
                                >
                                    <EditIconNew />{" "}
                                    {__("Edit Cookie", "surfalert")}
                                </li>
                                {/* @ts-ignore  */}
                                <li onClick={(event) => onDelete(event)}>
                                    {" "}
                                    <TrashIcon />{" "}
                                    {__("Delete Cookie", "surfalert")}{" "}
                                </li>
                            </ul>
                        </div>
                    )}
