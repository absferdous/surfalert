...
const PreviewButton = ({
    ...
    if (config.advance_edit) {
        ...
        // Add Border Radius
        if (
            config.nx_bar_border_radius_left ||
            config.nx_bar_border_radius_right ||
            config.nx_bar_border_radius_top ||
            config.nx_bar_border_radius_bottom
        ) {
            styles.borderRadius = `${config.nx_bar_border_radius_top || 0}px ${
                config.nx_bar_border_radius_right || 0
            }px ${config.nx_bar_border_radius_bottom || 0}px ${
                config.nx_bar_border_radius_left || 0
            }px`;
        }
    }
    ...
    if (config.source == "press_bar") {
        return (
            <>
                <div className="surfalert-link-wrapper" style={styles}>
                    <a
                        href={link}
