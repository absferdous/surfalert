...
const GDPR = (props) => {
    ...
    const handleGDPRBanner = () => {
        const areCookiesSet = document.cookie
            .split(";")
            .some((cookie) => cookie.trim().startsWith(`nx_cookie_manager=`));
        // @ts-ignore
    ...
    useEffect(() => {
        ...
        const isConsentIncomplete = Object.values(consent).some(
            (value) => value === false
        );
    ...
    let baseClasses = [
        ...
        settings.themes,
        settings?.themes?.includes("banner")
            ? `banner-gdpr banner-gdpr-${settings?.gdpr_theme}`
            : `card-gdpr card-gdpr-${settings?.gdpr_theme}`,
        settings?.gdpr_theme ? "dark" : "light",
        `sa-gdpr-${settings.nx_id}`,

        {
            ...
            "sa-gdpr-has-elementor": settings?.elementor_id,
            "sa-gdpr-has-gutenberg": settings?.gutenberg_id,
        },
    ];
    ...
    if (
        (is_pro && settings?.animation_notification_show !== "default") ||
        (is_pro && settings?.animation_notification_hide !== "default")
    ) {
        let animate_effect;
        if (
            settings?.animation_notification_hide !== "default" &&
            settings?.animation_notification_show === "default"
        ) {
            if (animation) {
            ...
            animate_effect = animation
                ? settings?.animation_notification_hide
                : settings?.animation_notification_show;
        }
    ...
    const wrapper = (
        // @todo advanced style.
        <div id={`sa-gdpr-${settings.nx_id}`} className={componentClasses}>
            <div className="sa-gdpr">
                <div
                    className={`sa-gdpr-card ${
                        settings?.disable_powered_by ? "no-branding" : ""
                    }`}
                >
                    {/* Header Section */}
                    <div className="sa-gdpr-card-header" style={mainBGColor}>
                        {settings?.gdpr_custom_logo && (
                            <img
                                src={settings?.gdpr_custom_logo?.url}
                                alt={settings?.gdpr_custom_logo?.title}
                                className="sa-gdpr-logo"
                            />
                        )}
                        <h3 className="sa-gdpr-title" style={titleColorFont}>
                            {settings?.gdpr_title}
                        </h3>
                    </div>

                    {/* Content Section */}
                    <div className="sa-gdpr-card-body" style={mainBGColor}>
                        <p
                            className="sa-gdpr-description"
                            style={descColorFont}
                        >
                            {settings?.gdpr_message}
                            {settings?.gdpr_cookies_policy_toggle &&
                                settings?.gdpr_cookies_policy_link_url && (
                                    <a
                                        href={
                                            settings?.gdpr_cookies_policy_link_url
                                        }
                                        target="_blank"
                                        className="sa-gdpr-link"
                                    >
                                        {
                                            settings?.gdpr_cookies_policy_link_text
                                        }
                                    </a>
                                )}
                        </p>
