...
const NotificationContainer = (props: any) => {
    ...
    const renderNotice = (NoticeList, position) => {
        ...
        const hasCustomAnimation = NoticeList.some(
            (item) =>
                item.config.animation_notification_hide !== "default" ||
                item.config.animation_notification_show !== "default"
        );
        ...
        return (
            <div
                className={`sa-container nxc-${position}`}
    ...
    return (
        <>
            {frontendContext.getNxToRender((position, NoticeList) => {
                if (
                    NoticeList?.[0]?.config?.type == "notification_bar" &&
                    (position == "top" || position == "bottom")
                ) {
                    return NoticeList.map((nxBar) => {
                        const nxId = nxBar?.config?.nx_id;
                        const reappearance = nxBar?.config?.bar_reappearance;
                        const countRand = nxBar?.config?.countdown_rand
                            ? `-${nxBar.config.countdown_rand}`
                            : "";
                        const storageKey = `notificationx_${nxId}${countRand}`;
                        if (
                            (reappearance === "dont_show_welcomebar" &&
                                localStorage.getItem(storageKey)) ||
                            (reappearance === "show_welcomebar_next_visit" &&
                ...
                if (position.indexOf("surfalert-shortcode-") === 0) {
                    return (
                        <Shortcode
                            key={`shortcode-${position}`}
                            position={position}
                        >
