...
const Pressbar = ({ position, nxBar, dispatch }) => {
    ...
    const target = usePortal(`sa-bar-${position}`, position == "top");
    ...
    const [isLoading, setIsLoading] = useState(
        settings.is_gutenberg && settings.gutenberg_id
    );
    ...
    const analyticCallback = useCallback((event) => {
        ...
        analyticsOnClick(
            ...
            restUrl,
            settings,
            frontendContext.rest.omit_credentials
        );
    }, []);
    ...
    const getTime = (settings) => {
        ...
        let countRand = settings?.countdown_rand
            ? `-${settings.countdown_rand}`
            : "";
        if (settings?.evergreen_timer) {
            let startedAt = cookie.load(
                `pressbar-evergreen-started-at-${settings.nx_id}${countRand}`
            );
            if (!startedAt) {
                startedAt = currentTime;
                const expires = new Date();
                expires.setDate(
                    expires.getDate() + (settings?.time_reset ? 1 : 365)
                );
                cookie.save(
                    `pressbar-evergreen-started-at-${settings.nx_id}${countRand}`,
                    currentTime,
            ...
            if (settings?.time_randomize) {
                let timeBetween = cookie.load(
                    `pressbar-evergreen-random-expire-${settings.nx_id}${countRand}`
                );

                if (!timeBetween) {
                    const start_time =
                        settings?.time_randomize_between?.start_time;
                    const end_time = settings?.time_randomize_between?.end_time;
                    timeBetween = Math.round(
                        Math.random() * (end_time - start_time) + start_time
                    );
                    const expires = new Date();
                    expires.setDate(
                        expires.getDate() + (settings?.time_reset ? 1 : 365)
                ...
                expiredTime =
                    parseInt(startedAt) + timeBetween * 60 * 60 * 1000;
            } else {
                expiredTime =
                    parseInt(startedAt) +
                    settings?.time_rotation * 60 * 60 * 1000;
            }
        } else {
            expiredTime =
                frontendContext
                    .getTime(settings.countdown_end_date || undefined)
                    .unix() * 1000;
            const startTime =
                frontendContext
                    .getTime(settings.countdown_start_date || undefined)
                    .unix() * 1000;
    ...
    const calcHeight = () => {
        ...
        if (
            ...
            // @ts-ignore
            const elements =
                elementorRef?.current?.getElementsByClassName(
                    "elementor-element"
                );
        ...
        if (settings?.advance_edit) {
            if (settings?.bar_bg_color)
                componentCSS.background = settings.bar_bg_color;
            if (settings?.bar_bg_image?.url) {
                componentCSS.backgroundImage = `url('${settings.bar_bg_image.url}')`;
            }
            if (settings?.bar_text_color)
                componentCSS.color = settings.bar_text_color;
            if (settings?.bar_font_size)
                componentCSS.fontSize = settings.bar_font_size;
            if (settings?.bar_btn_bg)
                buttonCSS.backgroundColor = settings.bar_btn_bg;
            if (settings?.bar_btn_text_color)
                buttonCSS.color = settings.bar_btn_text_color;
            if (settings?.bar_counter_bg)
                counterCSS.backgroundColor = settings.bar_counter_bg;
            if (settings?.bar_counter_text_color)
                counterCSS.color = settings.bar_counter_text_color;
            if (settings?.bar_close_color)
                closeButtonCSS.fill = settings.bar_close_color;
        }
        const barHeight = document.getElementById(
            `sa-bar-${settings.nx_id}`
        ).offsetHeight;

        document.body.classList.add("has-sa-bar");
        if (settings?.sticky_bar) {
            document.body.classList.add("sa-sticky-bar");
        }
        if (settings?.pressbar_body) {
            document.body.classList.add("sa-overlapping-bar");
        }
        if (position == "top") {
            document.body.classList.add("sa-position-top");
            const xAdminBar = document.getElementById("wpadminbar");
            if (xAdminBar?.offsetHeight)
        ...
        if (elementorRef?.current) {
            analyticDelegation = delegate(
                elementorRef?.current,
                "a",
                "click",
                analyticCallback
            );
            consentDelegation = delegate(
                elementorRef?.current,
                "#sa-consent-accept",
                "click",
                consentCallback
        ...
        return () => {
            ...
            document.body.classList.remove("sa-sticky-bar");
            document.body.classList.remove("sa-overlapping-bar");
            document.body.classList.remove("sa-position-top");
    ...
    useEffect(() => {
        ...
        document.addEventListener = function (
            ...
            originalAddEventListener.apply(document, [
                type,
                listener,
                options,
                ...args,
    ...
    const direction =
        settings?.bar_transition_style == "slide_right" ? "right" : "left";
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setCurrentSlide(
                (prevIndex) => (prevIndex + 1) % slidingContent.length
            );
        }, slideInterval);
    ...
    if (settings?.elementor_id) {
        innerContent = (
            <div
                ref={elementorRef}
                className="sa-bar-content-wrap"
                dangerouslySetInnerHTML={{
                    __html: addParentSelectorToCSS(content),
                }}
            ></div>
        );
        ...
        innerContent = (
            <div className="sa-bar-content-wrap">
                {settings?.enable_countdown && (
                    <div className="sa-countdown-wrapper">
                        {!timeConfig.expired && settings?.countdown_text && (
                            <div className="sa-countdown-text">
                                {__(settings?.countdown_text, "surfalert")}
                            </div>
                        )}
                        <div
                            className="sa-countdown"
                            style={styles?.counterCSS}
                        >
                            {((!timeConfig.expired && isTimeBetween) ||
                                (!timeConfig.expired &&
                                    settings?.evergreen_timer)) && (
                                <>
                                    <div
                                        className="sa-time-section"
                                        style={styles?.counterCSS}
                                    >
                                        <span className="sa-days">
                                            {timeConfig.days}
                                        </span>
                                        <span className="sa-countdown-time-text">
                                            {__("Days", "surfalert")}
                                        </span>
                                    </div>
                                    <div
                            ...
                            <span
                                className={classNames("sa-expired-text", {
                                    "sa-countdown-expired": timeConfig.expired,
                                })}
                                style={styles?.counterCSS}
                            >
                                {timeConfig.expired &&
                ...
                <div className="sa-inner-content-wrapper">
                    {settings?.bar_content_type == "static" && hasContent && (
                        <div
                            className="sa-bar-content"
                            dangerouslySetInnerHTML={{ __html: content }}
                        ></div>
                    )}
                    {settings?.bar_content_type === "sliding" &&
                        slidingContent.length > 0 && (
                            <div
                                className={classNames(
                                    "sa-bar-content sa-bar-slide-wrapper",
    ...
    const wrapper = (
        ...
        <div
            ...
            className={classNames(
                ...
                {
                    ["sa-bar-shortcode sa-bar-visible"]: isShortcode,
                    "sa-position-top": "top" == settings?.position,
                    "sa-position-bottom": "bottom" == settings?.position,
                    [`sa-close-${settings?.bar_close_position}`]:
                        settings?.bar_close_position,
                    "sa-admin": isAdminBar(),
                    "sa-sticky-bar": settings?.sticky_bar,
                    "sa-bar-has-elementor": settings?.elementor_id,
                    "sa-bar-has-gutenberg": settings?.gutenberg_id,
                    "sa-bar-default-design": !settings?.advance_edit,
                }
            )}
            ...
            style={{
                ...
                ...(themes_has_bg.includes(settings.themes)
                    ? {
                          background: settings.bar_bg_image?.url
                              ? `url('${settings.bar_bg_image.url}')`
                              : `url(${
                                    common_assets_url +
                                    settings.themes +
                                    ".webp"
