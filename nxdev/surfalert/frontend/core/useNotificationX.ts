...
const useNotificationX = (props: any) => {
    ...
    const getTime = (value?, keepLocalTime: boolean = false) => {
        const _value = moment
            ...
            .utcOffset(+props.config.gmt_offset, keepLocalTime);
        return _value;
    };
    ...
    useEffect(() => {
        if (props.config.nxPreview) {
            ...
            // let settings = JSON.parse(params.get('sa-preview'));
            ...
            if (Object.keys(config.pressbar).length) {
                ...
                setPressbarNotices(
                    normalizePressBar(filteredConfig, config.settings)
                );
            }
        ...
        if (activeNotices != null && activeNotices.length > 0) {
            activeNotices.forEach((entries) => {
                if (entries?.length > 0) {
                    ...
                    if (
                        !config?.random_order &&
                        config.source !== "custom_notification" &&
                        config.source !== "custom_notification_conversions"
                    ) {
                        sortArray(entries, {
                            by: "timestamp",
                            order: "desc",
        ...
        if (pressbarNotices != null && pressbarNotices.length > 0) {
            ...
            const checkAndShowNotice = (nxBar) => {
                ...
                const value = parseFloat(
                    config?.scroll_offset?.scroll_trigger_value || "300"
                );
                const appearCondition =
                    config?.appear_condition || "after_few_seconds"; // default: show immediately
                    ...
                    const timeoutID = setTimeout(() => {
                        ...
                        if (config?.auto_hide && +config?.hide_after) {
                            if (config?.close_forever) {
                                ...
                                const countRand = config?.countdown_rand
                                    ? `-${config.countdown_rand}`
                                    : "";
                                cookie.save(
                                    "notificationx_" +
                                        config?.nx_id +
    ...
    useEffect(() => {
        ...
        if (gdprNotices != null && gdprNotices.length > 0) {
            gdprNotices.forEach((gdprItem) => {
                ...
                const initialDelay =
                    (+config?.cookie_visibility_delay_before || 5) * 1000;
                const hideAfter = (+config?.hide_after || 5) * 1000;
                ...
                const timeoutID = setTimeout(() => {
                    ...
                    if (config?.auto_hide && +config?.hide_after) {
                        if (config?.close_forever) {
                            ...
                            let countRand = config?.countdown_rand
                                ? `-${config.countdown_rand}`
                                : "";
                            cookie.save(
    ...
    const getNxToRender = (callback: (position, NoticeList: []) => void) => {
        ...
        let fixedOrder = [
            ...
            "top_right",
            "center",
            "cookie_notice_bottom_left",
            "cookie_notice_bottom_right",
            "cookie_notice_center",
            "cookie_banner_top",
            "cookie_banner_bottom",
        ];
        for (let i = 0; i < state.notices.length; i++) {
            ...
            if (get_position.startsWith("surfalert-shortcode-")) {
    ...
    return {
        ...
        getTime,
        assets: {
            free: props.config.assets,
            pro: props.config?.pro_assets,
            common: props.config?.common_assets,
        },
