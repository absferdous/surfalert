...
// apiFetch.use(apiFetch.createNonceMiddleware(surfalert.rest.nonce));
...
export const isNotClosed = (entry) => {
    ...
    if (nx_id) {
        let countRand = entry?.post?.countdown_rand
            ? `-${entry.post.countdown_rand}`
            : "";
...
export const normalizeResponse = (response: any) => {
    ...
    let mergedShortcodeArray = normalize(
        response?.shortcode,
        response?.settings
    );
...
export const normalize = (_entries, globalSettings) => {
    ...
    for (const key in _entries) {
        if (Object.hasOwnProperty.call(_entries, key)) {
            ...
            let template;
            template = settings?.template_adv
                ? settings?.advanced_template?.split?.(/\r\n|\r|\n/)
...
export const normalizePressBar = (_entries, globalSettings) => {
    ...
    for (const key in _entries) {
        ...
        if (isNotClosed(entry)) {
            mergedArray = [
                ...mergedArray,
                { ...entry, post: { ...globalSettings, ...entry.post } },
            ];
        }
    }
    return mergedArray;
