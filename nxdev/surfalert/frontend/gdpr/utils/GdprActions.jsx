...
const GdprActions = ({ settings, onConsentGiven, setIsVisible }) => {
        ...
        const styles = settings?.advance_edit
            ? {
                  accept: {
                      ...
                      ...(settings?.gdpr_accept_btn_border_color && {
                          border: `1px solid ${settings.gdpr_accept_btn_border_color}`,
                      }),
                  },
                  customize: {
                      ...
                      ...(settings?.gdpr_customize_btn_border_color && {
                          border: `1px solid ${settings.gdpr_customize_btn_border_color}`,
                      }),
                  },
                  reject: {
                      ...
                      ...(settings?.gdpr_reject_btn_border_color && {
                          border: `1px solid ${settings.gdpr_reject_btn_border_color}`,
                      }),
                  },
              }
    ...
    const deleteCookies = async () => {
        try {
            await nxHelper.get(
                `index.php?rest_route=/surfalert/v1/delete-cookies/`
            );
        } catch (err) {
    ...
    const handleCustomizedConsent = () => {
        ...
        if (enabledItem.functional)
            loadScripts(settings?.functional_cookie_lists);
        if (enabledItem.analytics)
            loadScripts(settings?.analytics_cookie_lists);
        if (enabledItem.performance)
            loadScripts(settings?.performance_cookie_lists);
        if (enabledItem.advertising)
            loadScripts(settings?.advertising_cookie_lists);
        if (enabledItem.uncategorized)
            loadScripts(settings?.uncategorized_cookie_lists);
    };
        ...
        <Fragment>
            <div className="sa-gdpr-actions">
                ...
                <ReactModal
                    ...
                    className="sa-gdpr-customization"
                    style={modalStyle}
                    ariaHideApp={false}
                    portalClassName={"sa-gdpr-modal-wrapper"}
                >
                    <Customization
                        settings={settings}
