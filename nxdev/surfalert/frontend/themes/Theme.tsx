...
const Theme = (props) => {
            ...
            if (key === "time") {
                ...
                */
                ...
                if (randomNumber) {
                    ...
                    setRandomNumber(val);
                    Cookies.set("randomNumber", val.toString(), {
                        expires: expiry / (24 * 60),
                    }); // Cookie will expire after expiry minutes
    ...
    const componentClasses = classNames(
        ...
        // `sa-notification-${themeName}`,
        // {
        //     "sa-has-close-btn": post?.close_button,
        //     "has-no-image": entry?.image_data === false,
        // },
        // // Themes >> Advanced Edit
        // {
        //     [`sa-customize-style-${post?.id}`]: post?.advance_edit,
        //     [`sa-img-${post?.image_position}`]: post?.advance_edit,
        //     [`sa-img-${post?.image_shape}`]: post?.advance_edit,
        //     "sa-flex-reverse":
    ...
    if (post?.advance_edit) {
        ...
        // Add announcementCSS
        if (post.discount_text_color)
            announcementCSS.discountTextColor = post.discount_text_color;
        if (post.discount_background)
            announcementCSS.discountBackground = post.discount_background;
        if (post.link_button_bg_color)
            announcementCSS.linkButtonBgColor = post.link_button_bg_color;
        if (post.link_button_font_size)
            announcementCSS.linkButtonFontSize = post.link_button_font_size;
        if (post.link_button_text_color)
            announcementCSS.linkButtonTextColor = post.link_button_text_color;
    }
    ...
    return (
        ...
        >
            ...
            {["announcements_theme-15"].includes(props?.config?.themes) && (
                <Button {...props} announcementCSS={announcementCSS} />
            )}
            <Close {...props} />
        </div>
    );
